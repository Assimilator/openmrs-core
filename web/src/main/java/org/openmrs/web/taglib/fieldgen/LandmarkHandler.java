...
/**
 * This is the controller for the /web/web-inf/tags/landmarkField.tag.
 */
public class LandmarkHandler extends AbstractFieldGenHandler implements FieldGenHandler {
	
	private String defaultUrl = "landmark.field";
	
	/**
	 * @see FieldGenHandler#run()
	 */
	public void run() {
		setUrl(defaultUrl);
		...
		if (fieldGenTag != null) {
			...
			if (l != null) {
				...
				// (an example of when the dev doesn't want a default value is if landmark
			...
			LandmarkService cs = Context.getLandmarkService();
			List<Landmark> landmarks = cs.getAllLandmarks();
			...
			setParameter("landmarks", landmarks);
		}
	}
}
