			...
			<table class="regimenCompletedTable">
				<thead>
					<tr class="regimenCompletedHeaderRow">
						<th style="nowrap: true;" class="regimenCompletedDrugOrderedHeader"> <openmrs:message code="Order.item.ordered" /> </th>
					...
					<tr class="regimenCompletedHeaderOtherRow">
						<c:choose>
							<c:when test="${model.completedRegimenMode == 'view'}">
								<td colspan="6" class="regimenCompletedViewModeData"><table class="regimenCompletedViewModeInnerTable"><tr class="regimenCompletedViewModeInnerRow"><td class="regimenCompletedViewModeInnerData"><openmrs:message code="DrugOrder.header.otherRegimens" /></td></tr></table></td>
							</c:when>
							<c:otherwise>
								<td colspan="8" class="regimenCompletedOtherModeData"><table class="regimenCompletedOtherModeInnerTable"><tr class="regimenCompletedOtherModeInnerRow"><td class="regimenCompletedOtherModeInnerData"><openmrs:message code="DrugOrder.header.otherRegimens" /></td></tr></table></td>
							</c:otherwise>
						</c:choose>
					</tr>
				</tbody>
				<tbody id="regimenTableCompleted___other__">
									...
									<td class="regimenCompletedOtherModeData">
										...
										<div id="void_${drugOrder.orderId}" style="display:none" class="dashedAndHighlighted">
											<form class="regimenCompletedReasonForm">
												...
												&nbsp;&nbsp;
												<input type="button" value="<openmrs:message code="DrugOrder.void" />" onClick="handleVoidCompletedDrugOrder('${drugOrder.orderId}', 'void_${drugOrder.orderId}_reason')" />
					...
					<c:if test="${empty model.completedDrugOrderSets['*']}">
						<tr class="completedDrugOrderSetsRow">
							<c:choose>
								<c:when test="${model.completedRegimenMode == 'view'}">
									<td colspan="8" class="completedRegimenViewModeData"><span class="noOrdersMessage">&nbsp;&nbsp;&nbsp;&nbsp;(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:when>
								<c:otherwise>
									<td colspan="9" class="completedRegimenOtherModeData"><span class="noOrdersMessage">&nbsp;&nbsp;&nbsp;&nbsp;(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:if>
				...
				<tbody id="regimenTableCompleted_${fn:replace(drugSetId, " ", "_")}">
					...
					<c:if test="${empty model.completedDrugOrderSets[drugSetId]}">
						<tr class="noDrugsOrderRow">
							<c:choose>
								<c:when test="${model.completedRegimenMode == 'view'}">
									<td colspan="8" class="noDrugsOrderViewModeData"><span class="noOrdersMessage">&nbsp;&nbsp;&nbsp;&nbsp;(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:when>
								<c:otherwise>
									<td colspan="9" class="noDrugsOrderOtherModeData"><span class="noOrdersMessage">&nbsp;&nbsp;&nbsp;&nbsp;(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:if>
						...
						<tr class="patientRegimenDrugsNoOrderRow">
							<c:choose>
								<c:when test="${model.currentRegimenMode == 'view'}">
									<td colspan="8"><span class="noOrdersMessage">(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:when>
								<c:otherwise>
									<td colspan="9"><span class="noOrdersMessage">(<openmrs:message code="DrugOrder.list.noOrders" />)</span></td>
								</c:otherwise>
							</c:choose>
						</tr>
					</c:if>
							...
							function(i, e) { 
								e.onclick=function() {
									$j("#openmrs_dwr_error").show("slow", function () {
										$j("#openmrs_dwr_error_msg").text("You can add, delete or stop drug orders only.");
									});
